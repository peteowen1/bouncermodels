...
test_that("vb_publish restores piggyback_cache_duration on exit instead of leaking it for the rest of the session", {
  # Was a bare Sys.setenv(piggyback_cache_duration = 1) with no restore --
  # every unrelated caller after the first publish in a session silently ran
  # with piggyback's listing cache disabled. Check both starting states: unset,
  # and previously set to something else.
  dir <- withr::local_tempdir()
  p <- write_fixture_file(dir, "envfile.rds", content = "env-test-content")
  sz <- file.size(p)
  ...
  testthat::local_mocked_bindings(
    gh = function(...) list(assets = list(
      list(name = "envfile.rds", size = sz, updated_at = "2026-01-01T00:00:00Z", id = 1)
    )),
    .package = "gh"
  )

  withr::local_envvar(c(piggyback_cache_duration = NA))
  vb_publish(p, repo = "test/fixture", tag = "env-regression-tag-unset",
            carry_forward = FALSE, max_retries = 0)
  expect_identical(Sys.getenv("piggyback_cache_duration", unset = "__unset__"), "__unset__")

  withr::local_envvar(c(piggyback_cache_duration = "300"))
  vb_publish(p, repo = "test/fixture", tag = "env-regression-tag-set",
            carry_forward = FALSE, max_retries = 0)
  expect_identical(Sys.getenv("piggyback_cache_duration"), "300")
})
